...
from utilities import dicoLayerNode
from utilities import cylinder
...
def get_position(G, base_pos=None, layer_vertical_shift=2,
                 layer_horizontal_shift=0.0, proj_angle=45):
    ...
    for l in range(G.get_number_of_layers()):
        ...
        for j in range(G.get_layer(l).number_of_nodes()):
            pos[l*N+j] = np.array(
                    [pos[j][0]+ll, pos[j][1]+l*layer_vertical_shift],
                    dtype=np.float32)
    return pos

def get_position3D(G, base_pos=None, x_shift = 0,
                 y_shift = 0.0, z_shift = 1):
    """Return the position of the nodes.
    Parameters:
    -----------
    base_pos: position of base graph defualt value is None and thus function
    ...
    if base_pos is None:
        base_pos = [nx.layout.circular_layout(G.get_layer(i), dim = 3) for i in range(G.get_number_of_layers())]
    else:
        base_pos = base_pos
...
def OneLayer(ax,pos,graph, intra_edge_color = 'black',
             node_color = 'white', node_size = 25):
...
def FigureByLayer(multinet, pos = None, ax = None, intra_edge_color = 'black', 
             inter_edge_color = 'grey', node_color = 'white', 
             node_size = 25):
    ...
    if len(intra_edge_color) != L \
    and not colors.is_color_like(intra_edge_color):
        raise ValueError('intra_edge_code must be a color or a size number of layers list')
    
    # Assertion : layer_node_color is valid
    if len(node_color) != L \
    and not colors.is_color_like(node_color):
...
#                       ‘fantasy’ | ‘monospace’ 
def Figure3D(multinet, pos = None, ax = None, node_linewidth = None, 
             edge_linewidth = 1, edge_style = 'solid',
             node_list = None, edge_list = None, node_shape = 'o',
             node_color = None, node_cmap = None, node_size = 25, 
             node_alpha = 1, node_vmin = None, node_vmax = None,
             edge_color = None, edge_cmap = None, edge_alpha = 1,
             edge_vmin = None, edge_vmax = None, label = None,
             with_labels = False, labels = None,
             font_size = 12, font_color = 'k', font_weight = 'normal',
    ...
    elif len(node_color) == N and all(colors.is_color_like(color) for color in node_color):
        {}# it's ok
    ...
    # If color values of the good size
    elif len(edge_color) == E and all(colors.is_color_like(color) for color in edge_color):
        {}# it's ok
    # If numerical values of the good size
    ...
    for e in edge_list:
        ...
        zedge=[zedge1,zedge2]
        # Add edge to plot
        ax.plot(xedge,yedge,zedge,color=edge_color[i],alpha=edge_alpha, 
                linewidth = edge_linewidth, linestyle = edge_style)
        i = i+1

    # Add nodes to the figure
    ax.scatter(x1,y1,z1,s=node_size,c=node_color,alpha=node_alpha,
               depthshade=False, marker=node_shape, 
               linewidths=node_linewidth, label=label)
...
def layerNetwork(multinet, ax = None, color = 'grey', pos_layer = None,
                 radius_coef = 0.05, weight = True):
    ...
    for l in range(L):
        p = Rectangle((pos_layer[l][0], pos_layer[l][1]),pos_layer[l][3],
                      pos_layer[l][4], color = color)
        ax.add_patch(p)
...
# weight              boolean, if True, consider weight
def projectionDisplay(multinet, ax = None, pos = None, node_color = None, 
               node_size = 25, loop = True, weight = True, z = 0):
    ...
    if not all(layer.number_of_nodes() == nb_nodes for layer in multinet.get_list_of_layers()) : 
    ...
    for i in range(nb_nodes):
        for j in range(nb_nodes):
            if network[i][j] != 0:
                ...
                zedge=[z,z]
                # Add edge to plot
                ax.plot(xedge,yedge,zedge, 
                        linewidth = network[i][j],
                        color = 'grey')
                
                if loop and i==j:
                    circle2 = plt.Circle((pos[i][0], pos[i][1]), 0.1, 
                                         color='k', fill=False)
