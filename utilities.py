...
def average_clustering_proj(multinet, nodes = None, 
                       weight = None, count_zeros = True):
    return nx.average_clustering(projection(multinet), nodes, 
                                 weight, count_zeros)
                                 
def square_clustering_proj(multinet, nodes = None):
    return nx.square_clustering(projection(multinet), nodes)
    
def eigenvector_centrality_proj(multinet, max_iter=100, tol=1e-06, 
                           nstart=None, weight='weight'):
    return nx.eigenvector_centrality(projection(multinet), max_iter, 
                                             tol, nstart, weight)